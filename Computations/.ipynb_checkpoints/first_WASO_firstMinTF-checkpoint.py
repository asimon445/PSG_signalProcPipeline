    ...
    if (f != 41):
        ...
        if PSG_fnum == Hyp_fnum:
         ...
         #   data_pre_sleep = before_sleep_data[:, where_pre_sleep_wakes]
            ...
            alphaTS = np.zeros((6,int(DSlen)),dtype=float)
            deltaTS = np.zeros((6,int(DSlen)),dtype=float)

            for elec in range(data_during_sleep.shape[0]):
                ix = 0
                elec_thetaTS = np.zeros(int(DSlen),dtype=float)
                elec_alphaTS = np.zeros(int(DSlen),dtype=float)
                elec_deltaTS = np.zeros(int(DSlen),dtype=float)
                ...
                for wind in range(0,data_during_sleep.shape[1],itwin):
                    ...
                    alphaTS[elec,ix] = alpha
                    deltaTS[elec,ix] = delta
                    
                    elec_thetaTS[ix] = theta   #this one is for computing slope
                    elec_deltaTS[ix] = delta   #this one is for computing slope
                    elec_alphaTS[ix] = alpha   #this one is for computing slope
            ...
            if thetaTS[1].shape[0] > 0:
                ...
                GlobalAlpha[f,:] = alphaTS.mean(axis=0)
                GlobalDelta[f,:] = deltaTS.mean(axis=0)

                ## compute the slope of theta change across the first min of the first WASO that exceeds 1 min
                ...
                SleepHealth[f+1,9] = slope
                 ...
                 ## compute the slope of delta change across the first min of the first WASO that exceeds 1 min
                xx = np.zeros(GlobalDelta[f,:].shape[0],dtype = float)
                for x in range(GlobalDelta[f,:].shape[0]):
                    xx[x] = int(x)

                slope, intercept, r_value, p_value, std_err = linregress([xx],[GlobalDelta[f,:]])

                SleepHealth[f+1,10] = slope
                
                 ## compute the slope of alpha change across the first min of the first WASO that exceeds 1 min
                xx = np.zeros(GlobalAlpha[f,:].shape[0],dtype = float)
                for x in range(GlobalAlpha[f,:].shape[0]):
                    xx[x] = int(x)

                slope, intercept, r_value, p_value, std_err = linregress([xx],[GlobalAlpha[f,:]])

                SleepHealth[f+1,11] = slope
